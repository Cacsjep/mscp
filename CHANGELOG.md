...
## [2.9.0] - 2026-05-05
- Add Metadata Display: **Export to CSV** for every render type (Lamp, Number, Gauge, Text, Line Chart, Table). An **Export ⤓** badge appears top-right of the widget in Playback mode (left of the time-window picker for chart/table renderers; floating beside the stale badge for the rest). Opens a two-column dialog with editable settings on the left (From / To, optional daily time-of-day window with overnight wrap-around, weekday combo with Custom 7-checkbox row, delimiter, decimal separator, timestamp format, header row toggle) and a live 500 ms-debounced preview on the right showing the first 200 matching rows out of the full count. Validation marks invalid timestamps red; Save as... is gated on a current, non-empty preview so an outdated CSV can never be saved. **Enable Export to CSV** master checkbox per widget (default on) so dashboards / video walls can opt out individually. Lamp exports include a resolved-label column; multi-line Line Chart exports use a wide format (one column per series).
- Add Metadata Display: **Trend (KPI) render type**. Compact KPI tile that pairs a big current value with an up / down / neutral arrow, a Δ% versus a configurable baseline, and an inline sparkline of the recent history. Lookback (sparkline + baseline range) and baseline lookback are independent settings; baseline timestamp is marked on the sparkline as a dashed vertical guide. Reuses the Number panel's Min / Max / direction / colors / unit so threshold tinting matches across renderers and the arrow color follows the same High-is-bad direction (high-is-good → up green / down red; flipped otherwise; near-zero Δ shows the neutral glyph). Archive-backed: same Live + Playback backfill machinery as Line Chart and Table — the tile seeds itself from the archive on first appearance, anchors at the playback cursor in Playback, and exports through the same CSV dialog.
- Add Metadata Display: **Multi-series Line Chart**. A single Line Chart widget can now plot up to 8 series — each with its own Topic / Source filters / Data key, display name, color, line type (Straight / Smooth / Step), thickness, fill, markers, Y-axis assignment (left or right; right axis only renders when at least one series uses it), and an independent threshold band painted on its own axis. The configuration window keeps the existing fields as Series 1 and adds an "Additional series" accordion below them with a "+ Add series" button up to the 8-series cap; each row is a collapsible Expander with its own threshold sub-panel and Remove button. Optional **Show series legend strip** above the chart shows colored chips with display names; clicking a chip toggles that line on/off (session-only). Multi-series widgets walk the metadata XML once per packet (single parse fans out to every matching series) and run a single archive scan per backfill — the cost stays close to a one-line chart's. Existing single-series widgets auto-migrate; the saved JSON blob is empty until the operator adds a second series, so older builds continue reading the legacy single-line fields.
