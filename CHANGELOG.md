...
## [2.5.0] - 2026-05-01
- Add Timeline Jump: New Smart Client toolbar plugin (partner request) for jumping the playback timeline backward or forward by a chosen increment without dragging or scrubbing. Quick chips for `10s`/`30s`/`1m`/`10m` in both directions plus a custom Value/Unit picker (Seconds/Minutes/Hours/Days). The toolbar button opens a movable, dark-themed Smart Client styled flyout (drag the header, Esc to close). Jumps the master timeline by default, or only the selected tile when that tile is in independent playback. Visible in Live and Playback; the button stays disabled in Live until a tile is put into independent playback. **Snap to nearest recording**: if the target lands in an empty region, the jump snaps to the start of the next recorded sequence (forward) or the end of the previous one (backward) so the operator visibly moves to the next event. Compatible with XProtect 2022 R3 and newer.
